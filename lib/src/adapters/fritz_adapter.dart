...
class FritzBoxAdapter {
  ...
  Future<List<SmartDevice>> discoverDevices() async {
    ...
    final devices = <SmartDevice>[
      SmartDevice(
        ...
        capabilities: const {
          DeviceCapability.networkCounters,
          DeviceCapability.wifiClients,
        },
  ...
  Future<EnergyReading?> readEnergy(String smartDeviceId) async {
    ...
    await ensureSession();
    final stats = await _client.getEnergyStats(
    ...
    return EnergyReading(
      todayWh: stats.sumDay.toDouble(),
      monthWh: stats.sumMonth.toDouble(),
      raw: stats,
    );
  }
  ...
  Future<NetworkCounters?> readNetworkCounters() async {
    ...
    for (final url in urls) {
      ...
      final totals = _extractNetworkTotals(parsed);
      if (totals != null) {
        return totals;
      }
    }
  ...
  Future<List<_FritzDeviceHandle>> _loadDevices() async {
    ...
    return devices
        ...
        .map(_FritzDeviceHandle.fromJson)
        .whereNotNull()
        .toList();
  }
...
class _FritzDeviceHandle {
  ...
  static _FritzDeviceHandle? fromJson(Map<String, dynamic> json) {
    ...
    final units = (json['units'] as List?)?.whereType<Map<String, dynamic>>() ?? const Iterable<Map<String, dynamic>>.empty();
    final skills = units.expand((unit) => (unit['skills'] as List?)?.whereType<Map<String, dynamic>>() ?? const Iterable<Map<String, dynamic>>.empty()).toList();

    final temperature = _extractNumber(skills, const ['currentInCelsius', 'temperature', 'celsius']);
    final humidity = _extractNumber(skills, const ['humidity', 'currentRelativeHumidity', 'currentInPercent', 'relativeHumidity']);
...
NetworkCounters? _extractNetworkTotals(Map<String, dynamic> json) {
  ...
  void walk(dynamic value) {
    if (value is Map<String, dynamic>) {
      value.forEach((key, child) {
        ...
        if (child is num &&
            (lower.contains('bytes_sent') || lower.contains('bytesrcvd') || lower.contains('bytes_received') || lower == 'sum_bytes')) {
  ...
  final received = totals.entries.firstWhereOrNull((e) => e.key.contains('rcvd') || e.key.contains('received'))?.value ?? 0;
  final total = totals.entries.firstWhereOrNull((e) => e.key.contains('sum') || e.key.contains('total'))?.value ??
      (sent + received);
